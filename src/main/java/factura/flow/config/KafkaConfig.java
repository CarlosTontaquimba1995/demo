...
@EnableTransactionManagement // Add this line
public class KafkaConfig {
    ...
    public Map<String, Object> producerConfigs() {
        ...
        // Transaction and reliability settings
    ...
    public CommonErrorHandler kafkaErrorHandler() {
        DefaultErrorHandler errorHandler = new DefaultErrorHandler(
                ...
                new org.springframework.util.backoff.FixedBackOff(1000L, 3) // Reintentar 3 veces con 1 segundo de
                                                                            // espera
        );
    ...
    public ConcurrentKafkaListenerContainerFactory<String, PendingInvoiceDto> kafkaListenerContainerFactory(
        ...
        ConcurrentKafkaListenerContainerFactory<String, PendingInvoiceDto> factory = new ConcurrentKafkaListenerContainerFactory<>();
    ...
    @Bean
    public KafkaTemplate<String, PendingInvoiceDto> pendingInvoiceKafkaTemplate(
            ProducerFactory<String, Object> producerFactory) {
        // Create a producer factory specifically for PendingInvoiceDto
        DefaultKafkaProducerFactory<String, PendingInvoiceDto> pendingInvoiceProducerFactory = new DefaultKafkaProducerFactory<>(
        ...
        kafkaTemplate.setProducerListener(new ProducerListener<>() {
            ...
            public void onSuccess(
                    org.apache.kafka.clients.producer.ProducerRecord<String, PendingInvoiceDto> producerRecord,
                    org.apache.kafka.clients.producer.RecordMetadata recordMetadata) {
                if (recordMetadata != null) {
                    log.debug(
                            "Mensaje de factura pendiente enviado exitosamente - Tópico: {}, Partición: {}, Offset: {}",
                            recordMetadata.topic(), recordMetadata.partition(), recordMetadata.offset());
                }
            }
            ...
            public void onError(
                    ...
                    @Nullable org.apache.kafka.clients.producer.RecordMetadata recordMetadata,
                    Exception exception) {
                log.error("Error al enviar mensaje de factura pendiente - Tópico: {}", producerRecord.topic(),
                        exception);
            }
        });
        return kafkaTemplate;
